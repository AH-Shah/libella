...
def _remap_and_norm(
    ...
    barcodes = np.array(adata_mem.obs_names.astype(str))
    ...
    return X_raw_sparse, X_in_sparse, coords, barcodes

def build_pt_graph(f: Path, common_genes: list[str]) -> Path | None:
    """Master orchestrator for PyTorch Geometric graph building."""
    try:
        X_raw_sparse, X_in_sparse, coords, barcodes = _remap_and_norm(f, common_genes)
